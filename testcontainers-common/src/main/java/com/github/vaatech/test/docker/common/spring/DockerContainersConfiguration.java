...
public class DockerContainersConfiguration {
  ...
  public DockerContainersConfiguration(ConfigurableEnvironment environment) {
  ...
  public DockerEnvironment dockerContainers(
      @Autowired(required = false) ObjectProvider<DockerContainer[]> containersProvider)
      throws Exception {
...
//    Arrays.stream(containers).forEach(c -> ContainerUtils.logContainerInfo(c.name(), c, LOGGER));
    ...
    return new DockerEnvironment(
        allHealthy.call(), Arrays.stream(containers).toArray(GenericContainer<?>[]::new));
  }
}
